import pytest
from app.models import Franchise, Branch, StockItem, BranchInventory
...
def test_get_inventory_empty(client, setup_franchise_branch):
    ...
    response = client.get(f"/api/inventory?branch_id={b_id}", headers=branch_auth_headers)
...
def test_list_stock_items(client, setup_franchise_branch, db_session):
    ...
    response = client.get(f"/api/inventory/stock-items", headers=branch_auth_headers)
    assert response.status_code == 200
...
def test_add_branch_inventory(client, setup_franchise_branch, db_session):
    ...
    response = client.post("/api/inventory/transaction", json=trx_payload, headers=branch_auth_headers)
    assert response.status_code == 201
    
    inv = db_session.query(BranchInventory).filter_by(branch_id=b_id, stock_item_id=stock_item_id).first()
